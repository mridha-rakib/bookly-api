 ...
 * Customer Facebook flow; only the redirect URI differs
 * (FACEBOOK_PROFESSIONAL_OAUTH_REDIRECT_URI, its own "Valid OAuth Redirect URI" on the same Meta
 * app). Unset in dev/test — the start endpoint redirects the browser back with `status=error`
 * rather than crashing app boot.
 */
export function isProfessionalFacebookAuthConfigured(): boolean {
  return Boolean(
    env.FACEBOOK_CLIENT_ID &&
      env.FACEBOOK_CLIENT_SECRET &&
      env.FACEBOOK_PROFESSIONAL_OAUTH_REDIRECT_URI,
  );
}
...
function requireConfig(): FacebookOAuthConfig {
  const {
    ...
    FACEBOOK_PROFESSIONAL_OAUTH_REDIRECT_URI: redirectUri,
  } = env;

  if (!clientId || !clientSecret || !redirectUri) {
    throw new ProfessionalFacebookAuthError("PROFESSIONAL_FACEBOOK_AUTH_NOT_CONFIGURED", 503);
  }

  return { clientId, clientSecret, redirectUri };
}

export function buildProfessionalFacebookAuthUrl(state: string): string {
 ...
 * `email` may be absent — the service decides. Any provider failure collapses to
 * PROFESSIONAL_FACEBOOK_OAUTH_FAILED (502).
 */
export async function resolveProfessionalFacebookIdentity(
  code: string,
  ...
  try {
    ...
    if (error instanceof FacebookIdentityError) {
      throw new ProfessionalFacebookAuthError("PROFESSIONAL_FACEBOOK_OAUTH_FAILED", 502);
    }
    throw error;
  }
}
